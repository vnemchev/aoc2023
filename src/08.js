function solve(input) {
    ...
    for (let i = 0; i <= instructions.length; i++) {
        if (currentNode.node !== goalNode.node && i === instructions.length)
            i = 0;
        const instruction = instructions[i];
        let nextNode = {};

        if (instruction === 'R') nextNode = currentNode.nextRight;
        if (instruction === 'L') nextNode = currentNode.nextLeft;

        currentNode = nodes.find(n => n.node === nextNode);
        steps++;
