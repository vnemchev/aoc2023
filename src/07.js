...
function partOne(splitInput, splitInputL) {
    ...
    for (let i = 0; i < splitInputL; i++) {
        ...
        if (jokerHandType !== undefined) {
            console.log(`${handStr} - ${jokerHandType}`);
        } else {
        ...
        for (let j = 0; j < restOfInputL; j++) {
            ...
            if (nextJokerHandType !== undefined) {
                console.log(`${nextHandStr} - ${nextJokerHandType}`);
            } else {
                console.log(`${nextHandStr} - ${nextJokerHandType}`);
            }
...
function compareByCard(hand, nextHand) {
    ...
    for (let i = 0; i < handL; i++) {
        const cardPoints = getCardPoints(hand[i]);
        const nextCardPoints = getCardPoints(nextHand[i]);
