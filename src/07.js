function solve(input) {
    ...
    console.log(`Part 2: ${partTwo(splitInput, splitInputL)}`);
}
...
function parseRow(row) {
    ...
    let jokerHandType = undefined;
    if (hand.includes('J')) {
        jokerHandType = getJokerHandType(hand);
    }

    return [hand, handType, handStr, bid, jokerHandType];
}

function partTwo(splitInput, splitInputL) {
    const hands = {};
    let allRanks = 0;
    for (let i = 0; i < splitInputL; i++) {
        if (i === splitInputL - 1) break;
        const [hand, handType, handStr, bid, jokerHandType] = parseRow(
            splitInput[i],
        );
        let currentHandType;
        if (jokerHandType) currentHandType = jokerHandType;
        else currentHandType = handType;
...
function compareHands(
    ...
    if (handType === nextHandType) {
        const sortedHand = hand.toSorted(
            (a, b) => getCardPoints2(a) - getCardPoints2(b),
        );
        const sortedNextHand = nextHand.toSorted(
            (a, b) => getCardPoints2(a) - getCardPoints2(b),
        );
        if (compareByCard(sortedHand, sortedNextHand, flag))
            handsObj[handStr].wins++;
        else handsObj[nextHandStr].wins++;
    }
    return handsObj;
}

function compareByCard(hand, nextHand, flag) {
    const handL = hand.length;
    for (let i = 0; i < handL; i++) {
        let cardPoints = 0;
        let nextCardPoints = 0;
        if (flag === 'joker') {
            cardPoints = getCardPoints2(hand[i]);
            nextCardPoints = getCardPoints2(nextHand[i]);
        } else {
            cardPoints = getCardPoints(hand[i]);
            nextCardPoints = getCardPoints(nextHand[i]);
        }

        if (cardPoints > nextCardPoints) return true;
        if (cardPoints < nextCardPoints) return false;
    }
}
...
function getJokerHandType(hand) {
    const cards = hand.filter(
        card => hand.indexOf(card) !== hand.lastIndexOf(card) && card !== 'J',
    );
...
const getCardPoints2 = card =>
    ['J', '2', '3', '4', '5', '6', '7', '8', '9', 'T', 'Q', 'K', 'A'].indexOf(
        card,
    );
